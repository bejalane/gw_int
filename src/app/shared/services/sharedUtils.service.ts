...
import { SharedSettingsService } from './sharedSettings.service';
...
export class SharedUtilsService {
    ...
    getExcerpt(text, entity) {
        let res = text.split(' ');
        let excerpt: any = [];
        let excerptLength = this._sharedSettingsService.getExcerptLength(entity);
    ...
    isMobile() {
        ...
        // let isMobile = /iPhone|iPad|iPod|Android/i.test(this.deviceInfo.device);
    ...
    logoutProcedure() {
        SharedUtilsService.logoutClearStorage();
        this._notifierService.notifyOther({ option: 'checkLoggedUser', value: 'logout' });
    }
}
